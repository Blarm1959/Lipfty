...
// Lipfty 9 analysis-only optimized one-ply tactical comparison.
//
...
// Cheap, fixed H/V/D positional tie-breaker. Unlike baseActionPositionalScore(),
// this performs NO state lookahead and NO handover scan.
...
function neutralActionScore(s,a) { return staticCoreActionScore(s,a); }
...
// Immediate next-decision information only. No recursive action profiling.
...
function profileAction(s,a,lockedIds,rules) {
  ...
  if(actualWin) return {action:a,actualWin:true,replyWins:0,replyWinningColours:0,replyPotential:-Infinity,neutral:staticCoreActionScore(s,a)};
  let threat;
  ...
  return {action:a,actualWin:false,replyWins:threat.wins,replyWinningColours:threat.winningColours,replyPotential:threat.bestNeutral,neutral:staticCoreActionScore(s,a)};
}
...
function compareForActor(a,b) {
  ...
  // Positional potential is deliberately only the final tie-break.
  if(Math.abs(a.neutral-b.neutral)>1e-9) return b.neutral-a.neutral;
  return 0;
}
function bestProfilesForActor(profiles) {
  if(!profiles.length) return [];
  const sorted=[...profiles].sort(compareForActor),best=sorted[0];
...
function chooseActionLookahead(s,actions,lockedIds,rules,stats) {
  ...
  const profiles=[];
  for(let i=0;i<actions.length;i++) {
    profiles.push(profileAction(s,actions[i],lockedIds,rules));
    if((i&31)===0) heartbeat(`profile candidate actions ${i+1}/${actions.length}`);
  }
  const best=bestProfilesForActor(profiles),chosen=best[Math.floor(s.rng()*best.length)];
  stats.lookaheadActionDecisions++;
  const maxNeutral=Math.max(...profiles.map(x=>x.neutral));
  ...
  return {action:chosen.action,profile:chosen,mode:"one-ply"};
}

// Handover evaluation is intentionally cheap: actual immediate wins first,
// then the same static H/V/D score. It does not profile replies to every reply.
...
function chooseForcedSecondColour(s,lockedIds,rules,stats) {
  ...
  const options=colours.map(colour=>{
    ...
    return receiverDangerForColour(t,colour,lockedIds,rules,stats);
  });
...
function commitSequentialSecondLookahead(s,lockedIds,rules,stats) {
  ...
  const first=s.sequentialFirstColour,chosen=chooseForcedSecondColour(s,lockedIds,rules,stats);
...
// Responder's first compulsory placement after Move: take an immediate actual
// win if available; otherwise choose the first placement that leaves the mover
// the fewest immediate winning second placements. Static H/V/D breaks ties.
function commitMoveResponseLookahead(s,lockedIds,rules,stats) {
  ...
  if(count>=2 && s.responsePolicy==="sequential") {
    const candidates=[];
    const empties=emptySquares(s);
    for(const colour of availableNormalColours(s)) {
      for(let i=0;i<empties.length;i++) {
        const to=empties[i],a={type:"place",to,colour};
        const actualWin=actionWinsUnder(s,a,rules);
        const t=cloneState(s);
        ...
        if(!first.ended) {
          t.forcedQueue=[];
          secondThreat=nextDecisionThreat(t,lockedIds,rules);
        }
  ...
  // Rare exactly-one-reserve boundary: preserve actual immediate wins, then use
  // the common-core planner. This does not affect the main A/B/C comparison.
  if(count===1) {
...
function commitBoundaryCornerLookahead(s,lockedIds,rules,stats) {
  ...
  const options=[...new Set(s.finalPieces)].map(colour=>receiverDangerForColour(s,colour,lockedIds,rules,stats));
...
function redeployCandidateProfile(s,piece,to,lockedIds,rules) {
  ...
  if(actualWin) return {to,actualWin:true,nextWins:0,neutral:staticCorePlacementScore(b,piece.colour,to)};
  const t=cloneState(s),responder=s.currentPlayer;
  const first=S.applyRedeployPlacement(t,piece,to,rules);
  if(first.ended) return {to,actualWin:true,nextWins:0,neutral:staticCorePlacementScore(b,piece.colour,to)};
  t.forcedPlacements=0; t.forcedQueue=[];
  if(t.jumpConsequence==="redeploy-pass") t.currentPlayer=responder;
  const threat=nextDecisionThreat(t,lockedIds,rules);
  return {to,actualWin:false,nextWins:threat.wins,neutral:staticCorePlacementScore(b,piece.colour,to)};
}
function resolveJumpRedeployLookahead(s,jumpAction,lockedIds,rules,stats) {
  ...
  for(let i=0;i<empties.length;i++) {
    candidates.push(redeployCandidateProfile(s,piece,empties[i],lockedIds,rules));
...
selfTest();
if(hasArg("self-test")) { console.log("Lipfty 9 optimized one-ply tactical comparison self-test passed."); process.exit(0); }

const started=Date.now(); runStartedAt=started; heartbeatLast=started; archivePreviousResults();
console.log("");
console.log("Lipfty 9 — optimized one-ply tactical comparison");
console.log(`Run started: ${fmtDateTime(started)}`);
console.log(`Results: ${outDir}`);
console.log(`Matched seeds: ${seed}-${seed+games-1} (${games})`);
console.log("A/B/C use their real winning rules for immediate wins and optimized one-ply threat avoidance.");
console.log("The common static H/V/D score is used only after actual-rule one-ply ties.\n");
...
for(let i=0;i<games;i++) {
  ...
  for(const c of CONDITIONS) {
    heartbeatSeed=gameSeed; heartbeatCondition=c.id; heartbeat(`start condition`,true); const g=playGame(gameSeed,c,true); byId[c.id]=g; allGames.push(gameRow(g)); addAggregate(aggregates.get(c.id),g);
...
const summaryPath=path.join(outDir,`lipfty9-oneply-tactical-summary-${tag}.csv`);
const pairsPath=path.join(outDir,`lipfty9-oneply-tactical-pairs-${tag}.csv`);
const gamesPath=path.join(outDir,`lipfty9-oneply-tactical-games-${tag}.csv`);
const divergencePath=path.join(outDir,`lipfty9-oneply-tactical-divergences-${tag}.csv`);
const tracePath=path.join(outDir,`lipfty9-oneply-tactical-trace-${tag}.csv`);
const reportPath=path.join(outDir,`lipfty9-oneply-tactical-report-${tag}.txt`);
...
const report=[
  "Lipfty 9 — Optimized one-ply tactical comparison",
  ...
  "Candidate actions count only immediate actual-rule replies before any positional tie-break.",
  "The same cheap static H/V/D common-core score is used only as the final tie-breaker in all conditions.",
  ...
  "",
  "INTERPRETATION",
  "This optimized tester asks whether the square-rule changes remain biased when players explicitly see and avoid immediate consequences under the rules they actually have.",
