...
function defaultCsv(o){return path.join("C:\\bxd\\Lipfty-Simulation-Results",`lipfty7-redeploy-only-comparison-${o.strength}-${o.games}-seed${o.seed}.csv`);}
...
function printDelta(base,test){
  console.log("\nCHANGE: D — REDEPLOY ONLY, NO MOVE minus A — CURRENT");
...
function help(){
  console.log("Usage: node .\\tools\\run-lipfty7-redeploy-only-comparison.js [--games N] [--seed N] [--strength tactical|random] [--progress-every N] [--csv FILE|--no-csv]");
  console.log("Defaults: 5,000 games per configuration, tactical strength, seed 1, progress every 5%.");
  console.log("A = current Standard baseline with Move. D = Move disabled; a non-winning Jump hands the exact jumped piece to the responder, who immediately redeploys it; then normal play resumes with the jumper. No forced reserve placement follows the Jump.");
}
...
function main(argv=process.argv.slice(2)){
  ...
  console.log("Lipfty 7 Standard redeploy-only Jump experiment");
  ...
  console.log("D REDEPLOY ONLY, NO MOVE: Move OFF; jumped piece is immediately redeployed by opponent; no reserve piece is forced; normal play resumes with the jumper.");

  const a=runOne("A — CURRENT",standard,"current",o);
  const d=runOne("D — REDEPLOY ONLY, NO MOVE",noMove,"redeploy-only",o);
  printDelta(a.r,d.r);

  if(csvPath){
    writeCsv(csvPath,[flat("A-current",a.r,a.ms,a.rules),flat("D-redeploy-only-no-move",d.r,d.ms,d.rules)]);
