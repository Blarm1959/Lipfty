...
function defaultCsv(o){return path.join("C:\\bxd\\Lipfty-Simulation-Results",`lipfty7-jump-colour-comparison-${o.strength}-${o.games}-seed${o.seed}.csv`);}
...
function flat(label,r,elapsedMs){
  return{
    jump_rule:label,jump_policy:r.jumpPolicy,games:r.games,p1_wins:r.wins[0],p2_wins:r.wins[1],draws:r.draws,
...
function printDelta(opposite,any){
  console.log("\nCHANGE: ANY-COLOUR minus OPPOSITE-COLOUR");
  console.log(`  P1 win: ${(any.firstPlayerWinPct-opposite.firstPlayerWinPct).toFixed(2)} percentage points`);
  console.log(`  P2 win: ${(any.secondPlayerWinPct-opposite.secondPlayerWinPct).toFixed(2)} percentage points`);
  console.log(`  Draw: ${(any.drawPct-opposite.drawPct).toFixed(2)} percentage points`);
  console.log(`  P1 score: ${(any.firstPlayerScorePct-opposite.firstPlayerScorePct).toFixed(2)} percentage points`);
  console.log(`  Average turns: ${(any.averageTurns-opposite.averageTurns).toFixed(3)}`);
  console.log(`  Final Four frequency: ${(any.finalFourPct-opposite.finalFourPct).toFixed(2)} percentage points`);
  console.log(`  Move wins P1/P2: ${any.winningActionTypes.move[0]-opposite.winningActionTypes.move[0]} / ${any.winningActionTypes.move[1]-opposite.winningActionTypes.move[1]}`);
  console.log(`  Jump wins P1/P2: ${any.winningActionTypes.jump[0]-opposite.winningActionTypes.jump[0]} / ${any.winningActionTypes.jump[1]-opposite.winningActionTypes.jump[1]}`);
  console.log(`  Move frequency: ${any.moves-opposite.moves}; Jump frequency: ${any.jumps-opposite.jumps}; compulsory placements: ${any.forcedPlacements-opposite.forcedPlacements}`);
  console.log(`  Games with 2+ responses: ${any.gamesWithMultipleResponses-opposite.gamesWithMultipleResponses}; max responses/game: ${opposite.maxResponsesPerGame} -> ${any.maxResponsesPerGame}`);
}
...
function help(){
  console.log("Usage: node .\\tools\\run-lipfty7-jump-colour-comparison.js [--games N] [--seed N] [--strength tactical|random] [--progress-every N] [--csv FILE|--no-csv]");
  console.log("Defaults: 5,000 games per jump rule, tactical strength, seed 1, progress every 5%.");
  console.log("Only jump-over colour changes: opposite-colour versus any-colour. The Lipfty 7 committed two-piece response is used in both runs.");
}
function runOne(label,jumpPolicy,o,rules){
  console.log(`\nStarting ${label}...`);
  const start=process.hrtime.bigint();
  const r=S.runBatch({rules,games:o.games,seed:o.seed,strength:o.strength,jumpPolicy,progressEvery:progressStep(o),onProgress:progressReporter(label,start,o.games)});
...
function main(argv=process.argv.slice(2)){
  ...
  console.log("Lipfty 7 Standard Jump-colour experiment: OPPOSITE-COLOUR vs ANY-COLOUR");
  console.log(`${o.games} games each; strength ${o.strength}; base seed ${o.seed}; identical Standard rules and committed two-piece Move/Jump response.`);
  console.log("Only the colour of the piece that may be jumped over is changed. No playable app/rules are changed by this experiment.");

  const opposite=runOne("OPPOSITE-COLOUR JUMP", "opposite",o,rules);
  const any=runOne("ANY-COLOUR JUMP", "any",o,rules);
  printDelta(opposite.r,any.r);

  if(csvPath){writeCsv(csvPath,[flat("opposite-colour",opposite.r,opposite.ms),flat("any-colour",any.r,any.ms)]);console.log(`\nDetailed comparison CSV: ${csvPath}`);}
}
