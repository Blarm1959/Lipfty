...
function defaultCsv(o){return path.join("C:\\bxd\\Lipfty-Simulation-Results",`lipfty7-redeploy-next-turn-comparison-${o.strength}-${o.games}-seed${o.seed}.csv`);}
...
function flat(label,r,elapsedMs,rules){
  return{
    ...
    placement_wins_p1:r.winningActionTypes.placement[0],placement_wins_p2:r.winningActionTypes.placement[1],move_wins_p1:r.winningActionTypes.move[0],move_wins_p2:r.winningActionTypes.move[1],
    jump_direct_wins_p1:r.winningActionTypes.jump[0],jump_direct_wins_p2:r.winningActionTypes.jump[1],redeploy_wins_p1:r.winningActionTypes.redeploy[0],redeploy_wins_p2:r.winningActionTypes.redeploy[1],
    ...
    response_count_distribution:distribution(r.responseCountDistribution),games_with_multiple_responses:r.gamesWithMultipleResponses,max_responses_per_game:r.maxResponsesPerGame,elapsed_seconds:elapsedMs/1000
  };
}
...
function printResult(label,r,rules,elapsedMs){
  ...
  console.log(`  Redeploy wins: replayed jumped piece ${pair(r.redeployWins)} | forced reserve after Jump ${pair(r.jumpReserveWins)}`);
  ...
  console.log(`  Response count per game: ${distribution(r.responseCountDistribution)} | games with 2+ ${r.gamesWithMultipleResponses}, max ${r.maxResponsesPerGame}`);
...
function help(){
  console.log("Usage: node .\\tools\\run-lipfty7-redeploy-next-turn-comparison.js [--games N] [--seed N] [--strength tactical|random] [--progress-every N] [--csv FILE|--no-csv]");
  console.log("Defaults: 5,000 games per configuration, tactical strength, seed 1, progress every 5%.");
  console.log("A = current Standard baseline with Move. D = no Move, redeploy-only, jumper resumes. E = no Move, redeploy-only, responder also receives the next normal turn; jumper chooses the reserve colour handed to the responder.");
}
...
function main(argv=process.argv.slice(2)){
  ...
  console.log("Lipfty 7 Standard redeploy next-turn experiment");
  console.log(`${o.games} games each; strength ${o.strength}; base seed ${o.seed}.`);
  console.log("Fixed: opposite-colour Jump, 7.1 CURRENT responder-choice boundary, Standard win patterns.");
  console.log("A CURRENT: Move ON + current Jump consequence.");
  console.log("D REDEPLOY ONLY, JUMPER NEXT: Move OFF; opponent redeploys jumped piece; no reserve is forced; jumper takes next normal turn.");
  console.log("E REDEPLOY ONLY, RESPONDER NEXT: Move OFF; opponent redeploys jumped piece and also takes next normal turn; jumper chooses the reserve colour for that turn.");

  const a=runOne("A — CURRENT",standard,"current",o);
  const d=runOne("D — REDEPLOY ONLY, JUMPER NEXT",noMove,"redeploy-only",o);
  const e=runOne("E — REDEPLOY ONLY, RESPONDER NEXT",noMove,"redeploy-pass",o);
  printDelta("D — REDEPLOY ONLY, JUMPER NEXT",a.r,d.r);
  printDelta("E — REDEPLOY ONLY, RESPONDER NEXT",a.r,e.r);

  if(csvPath){
    writeCsv(csvPath,[
      flat("A-current",a.r,a.ms,a.rules),
      flat("D-redeploy-only-jumper-next",d.r,d.ms,d.rules),
      flat("E-redeploy-only-responder-next",e.r,e.ms,e.rules)
    ]);
